...
function show(req, res, next) {
  return _sqldb2.default.Campaign.findById(req.params.id).then(function (route) {
...
function destroy(req, res, next) {
  return _sqldb2.default.Campaign.destory({ where: { id: req.params.id } }).then(function () {
    return res.status(201).end();
  }).catch(next);
}
