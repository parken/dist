...
function update(req, res, next) {
  return _sqldb2.default.Group.update((0, _assign2.default)({}, req.body), { where: { id: req.params.id } }).then(function () {
    return res.status(201).end();
  }).catch(next);
}

function destroy(req, res, next) {
  return _sqldb2.default.Group.destory({ where: { id: req.params.id } }).then(function () {
    return res.status(201).end();
  }).catch(next);
}
