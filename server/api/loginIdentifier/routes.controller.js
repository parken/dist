...
function index(req, res, next) {
  ...
  return _promise2.default.all([_sqldb2.default.LoginIdentifier.findAll(options), _sqldb2.default.LoginIdentifier.count()]).then(function (_ref) {
...
function show(req, res, next) {
  return _sqldb2.default.LoginIdentifier.findById(req.params.id).then(function (route) {
    return res.json(route);
  }).catch(next);
}

function create(req, res, next) {
  return _sqldb2.default.LoginIdentifier.create((0, _assign2.default)({}, req.body, {
    userId: req.user.id
  })).then(function (_ref3) {
    var id = _ref3.id;
...
function update(req, res, next) {
  return _sqldb2.default.LoginIdentifier.update((0, _assign2.default)({}, req.body), { where: { id: req.params.id } }).then(function () {
    return res.status(201).end();
  }).catch(next);
}

function destroy(req, res, next) {
  return _sqldb2.default.LoginIdentifier.destory({ where: { id: req.params.id } }).then(function () {
    return res.status(201).end();
  }).catch(next);
}
